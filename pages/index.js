...
export default async function Page() {
    ...
    return (
        ...
        <div className="stocks">
          {Object.entries(stockData).map(([name, stock], index) => (
            <div
              key={index}
              className={`stock-card ${stock.down ? 'down' : 'up'}`} // Fixed here
            >
  ...
  async function getMarketData() {
    ...
    try {
        ...
        const amazonDoc = await db
        .collection('amazon')
        .findOne({}, { projection: { _id: 0, symbol: 1, price: 1, percentageChange: 1 } });
        const appleDoc = await db
        .collection('apple')
        .findOne({}, { projection: { _id: 0, symbol: 1, price: 1, percentageChange: 1 } });
        const netflixDoc = await db
        .collection('netflix')
        .findOne({}, { projection: { _id: 0, symbol: 1, price: 1, percentageChange: 1 } });
        const googleDoc = await db
        .collection('google')
        .findOne({}, { projection: { _id: 0, symbol: 1, price: 1, percentageChange: 1 } });
        ...
        console.error(err);
        return <Home stockData={{}} error="Failed to fetch stock data." />;
      } finally {
        if (client) {
          await client.close();
        }
